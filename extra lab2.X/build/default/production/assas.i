...
# 12 "assas.s" 2
 ...
 main:
  ...
  config_reloj:
   banksel OSCCON
   bcf ((OSCCON) and 07Fh), 4 ;Reloj en 100 correspondiente a 1Mhz
   bcf ((OSCCON) and 07Fh), 5
   bsf ((OSCCON) and 07Fh), 6
   return

   cont_8b:;Subrutina del contador de 8bits
     btfsc PORTB,0;Comprueba el estado de ((PORTB) and 07Fh), 0,si es 0 realiza un salto
    call inc_porta; Si el estado de ((PORTB) and 07Fh), 0 es 1 se ejecuta el call del incrementador
    btfsc PORTB,1;Comprueba el estado de ((PORTB) and 07Fh), 1,si es 0 realiza un salto
    call dec_porta; Si el estado de ((PORTB) and 07Fh), 1 es 1 se ejecuta el call para la función dec_porta
    return
   cont_4b:;Contador de 4bits que funciona igual al de 8 bits únicamente que se usan solo 4 salidas
    ...
    sumador:;Subrutina del sumador
     btfsc PORTB, 4;Comprueba el estado de ((PORTB) and 07Fh), 4,si es 0 realiza un salto
     call suma
    return
    ...
    bcf PORTE,0;Si el carry es 0 establece el ((TRISE) and 07Fh), 0 en cero
    btfsc carry,4;Evaluamos en 5to bit de la variable,correspondiente al carry, si es 0 salta
    call carry_bit;Si el carry es 1 establece el ((TRISE) and 07Fh), 0 en cero
    return
